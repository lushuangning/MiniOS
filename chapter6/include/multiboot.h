...
/**
 *
 * 启动后，在进入32位内核时，机器状态如下：
 *  1. CS 指向基地址 0x00000000，限长为 4G - 1 的代码段描述符。
 *  2. DS, SS, ES, FS 和 GS 指向基地址为 0x00000000，限长为 4G - 1 的代码段描述符。
 *  3. A20 地址线已经打开。
 *  4. 页机制被禁止。
 *  5. 中断被禁止。
 *  6. EAX = 0x2BADB002，这个值指出操作系统是被一个符合 Multiboot 规范的引导程序载入的。
...
typedef struct mmap_entry_t {
  uint32_t size;	    // 保留 size 是不含 size 自身变量的大小
